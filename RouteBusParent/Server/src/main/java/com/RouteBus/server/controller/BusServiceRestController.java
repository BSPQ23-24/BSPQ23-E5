...
public class BusServiceRestController {
	...
	@PutMapping("/bus/update/{licensePlate}")
	public ResponseEntity<Object> updateUser(@PathVariable String licensePlate, @RequestBody Bus bus) {
		log.warn(String.format("Updating bus with license plate {%s} ...", licensePlate));

		switch (busService.updateBus(bus, licensePlate)) {
		case FAIL:
			return ResponseEntity.unprocessableEntity()
					.body(String.format("Bus with license plate {%s} cannot be updated", bus.getLicensePlate()));
		default:
			return ResponseEntity.ok(String.format("Bus with email {%s} has been updated.", bus.getLicensePlate()));
		}
	}
	...
	@PutMapping("/station/update/{name}")
	public ResponseEntity<Object> updateStation(@PathVariable String name, @RequestBody Station station) {
		log.warn(String.format("Updating station with name {%s} ...", name));

		switch (busService.updateStation(station, name)) {
		case FAIL:
			return ResponseEntity.unprocessableEntity()
					.body(String.format("Station with name {%s} cannot be updated", station.getName()));
		default:
	...
	@PutMapping("/route/update/{name}")
	public ResponseEntity<Object> updateRoute(@PathVariable String name, @RequestBody Route route) {
		log.warn(String.format("Updating route with name {%s} ...", name));

		switch (busService.updateRoute(route, name)) {
		case FAIL:
			return ResponseEntity.unprocessableEntity()
					.body(String.format("Route with id {%i} cannot be updated", route.getName()));
		default:
			return ResponseEntity.ok(String.format("Route with name {%s} has been updated.", route.getName()));
		}
	}
	...
	public ResponseEntity<Object> addBusToRoute(@PathVariable Long routeId, @PathVariable Long busId) {
	...
	public ResponseEntity<Object> addStationToRoute(@PathVariable Long routeId, @PathVariable Long stationId) {
	...
	public List<Route> obtainRoutesByBus(@PathVariable Long busId) {
